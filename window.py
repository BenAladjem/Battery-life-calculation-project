...
lbl7 = Label(window, text="Report time ", font=("Arial Bold", 13))
...
txt_rep_freq.grid(row=18, column=4)
...
def print_r():
    global mode
    if len(mode) < 9 and not mode[-1] == "R":
        mode += "R"
...
def print_s():
    global mode
    if len(mode) < 9 and not mode[-1] == "S":
        mode += "S"
    lbl100.configure(text=mode)
...
def print_w():
    global mode
    if len(mode) < 9 and not mode[-1] == "W":
        mode += "W"
    lbl100.configure(text=mode)
...
btn_ws = Button(window, text="WS Cycles").grid(column=4, row=19)
